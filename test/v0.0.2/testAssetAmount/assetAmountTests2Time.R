...
testCallNumber10AssetNumberS20M20L10 <- function(loadFile=data1){
  ...
  for(i in 1:5){
    ...
    output <- cbind(paste(loadFile,"RData",sep='.'),timeLimit,runTime[i],analysis$dailyCost,analysis$reservedLiquidityRatio,analysis$movements)
...
testCallNumber20AssetNumberS20M20L10 <- function(loadFile=data2){  
  loadPath <- paste(paste(loadFolder,loadFile,sep = '/'),"RData",sep=".")
  load(loadPath)
