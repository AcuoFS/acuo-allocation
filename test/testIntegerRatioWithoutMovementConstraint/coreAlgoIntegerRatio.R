...
CoreAlgoV2 <- function(callInfo_df, resource_df, availInfo_list,
  ...
  if(1*(!is.element(0,ifSelectAssetSuff_vec))){
    ...
    #### OBJECTIVE FUNCTION
    liquidityObj_vec <-  c(minUnitValue_vec*objParams_list$liquidity_vec[idxEli_vec])
    costObj_vec <-  c(minUnitValue_vec*objParams_list$cost_vec[idxEli_vec])
    ...
    lpObj_vec <- fObj_vec
    lpCon_mat <- rbind(fCon2_list$fCon2_mat,fCon3_list$fCon3_mat)
    lpDir_vec <- c(fCon2_list$fDir2_vec,fCon3_list$fDir3_vec)
    lpRhs_vec <- c(fCon2_list$fRhs2_vec,fCon3_list$fRhs3_vec)
    
    
    lpKind_vec <- rep('semi-continuous',varNum)
    lpType_vec <- rep('real',varNum)
    lpType_vec[which(minUnitValue_vec>=1)] <- 'integer'
    lpLowerBound_vec <- c(minMoveQty_vec)
    ...
    lpUpperBound_vec <- c(quantity_vec)
    lpBranchMode_vec <- c(rep('auto',varNum))
    ...
    #### INITIAL GUESS BASIS 
    lpGuessBasis_vec <- rep(0,varNum)
    if(!missing(initAllocation_list)){
      # the initial guess must be a feasible point
      lpGuessBasis_vec<-ResultList2Vec(initAllocation_list,callId_vec,minUnit_vec,varName_vec,varNum)
    }
    #### Solver Inputs END ###################
    ...
    solverSolution_vec <- AdjustResultVec(solverSolution_vec,varNum,varName_vec,
