...
CostDefinitionActualCost <- function(availAsset_df){
  negIdx <- which(availAsset_df$opptCost < 0)
  if(length(negIdx)){
    availAsset_df$opptCost[negIdx] <- 0
  }
  cost <- availAsset_df$internalCost+availAsset_df$externalCost+availAsset_df$opptCost-(availAsset_df$interestRate+availAsset_df$yield)
  return(cost)
}
...
ResultAnalysisActualCost <- function(availAssetOri_df,availAsset_df,resourceOri_df,resource_df,callInfo_df,callOutput_list){
  callId_vec <- callInfo_df$id
  resource_vec <- resource_df$id
  resourceOri_vec <- resourceOri_df$id
  ...
  cost_mat <- availInfo_list$cost_mat
  cost_vec <- as.vector(t(cost_mat))[idxEli_vec]
  costActual_mat <- availInfo_list$costActual_mat
  costActual_vec <- as.vector(t(costActual_mat))[idxEli_vec]
  ...
  #### Costs
  dailyCost <- CostFun(varAmount_vec,costActual_vec)
  monthlyCost <- dailyCost*30
  dailyCost <- dailyCost
  monthlyCost <- monthlyCost
...
AssetByCallInfo <- function(callId_vec,resource_vec,availAsset_df){
  ...
  costActual_mat[cbind(idxTempCallId_vec,idxTempResource_vec)]<- CostDefinitionActualCost(availAsset_df)
  ...
  costActual_vec <- as.vector(t(costActual_mat))
