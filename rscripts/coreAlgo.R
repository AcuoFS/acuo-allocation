...
CoreAlgo <- function(coreInput_list,availAsset_df,pref_vec,timeLimit,verbose){
  pref_vec<-pref_vec
  ...
  if(!is.element(0,ifSelectAssetSuff_vec)){
    ...
    solverOutput_list <- CallLpSolve(lpObj_vec,lpCon_mat,lpDir_vec,lpRhs_vec,
                                     ...
                                     presolve=lpPresolve,epsd=lpEpsd,timeout=lpTimeout,verbose=lpVerbose,bbRule=bbRule)
    ### end ##################
